...
export function DocumentoProspettoStampa({ doc }: { doc: DocumentoProspetto }) {
  return (
    <article
      ...
      aria-hidden
      data-documento="prospetto"
    >
      <header className="break-inside-avoid border-b border-black/25 pb-3">
        <div className="flex items-baseline justify-between gap-4">
          <h1 className="font-display text-[15pt] font-semibold leading-tight">
            {doc.titolo}
          </h1>
          <span className="text-[8pt] uppercase tracking-wider text-black/50">Flowlance</span>
        </div>
        <p className="mt-0.5 text-[9pt] text-black/60">
          {doc.intestatario} · emesso il{" "}
          {doc.identificazione.find((v) => v.etichetta === "Documento emesso il")?.valore}
        </p>
      </header>

      <div className="mt-3 grid grid-cols-2 gap-x-6 break-inside-avoid">
      ...
      <section className="mt-3 break-inside-avoid border-y border-black/20 py-2">
      ...
      {doc.sezioni.map((sezione) => (
        <section key={sezione.id} className="mt-3 break-inside-avoid">
      ...
      <footer className="mt-4 break-inside-avoid border-t border-black/25 pt-2">
        <p className="text-[7.5pt] leading-snug text-black/60">{doc.nota}</p>
        {doc.fonti.length > 0 && (
          <p className="mt-1 text-[7.5pt] leading-snug text-black/50">
            <span className="font-medium">Parametri {doc.anno}:</span> {doc.fonti.join(" · ")}
          </p>
        )}
      </footer>
    </article>
  ...
  return (
    <tr className="border-b border-black/10 align-baseline">
      <td className="py-[3px] pr-3">
        ...
        {riga.formula && (
          <span className="block text-[7.5pt] leading-snug text-black/50">{riga.formula}</span>
        )}
      </td>
      <td
        className={`cifre w-[32%] py-[3px] text-right tabular-nums ${
